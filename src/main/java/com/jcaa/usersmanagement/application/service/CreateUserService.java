...
public final class CreateUserService implements CreateUserUseCase {
  ...
  public UserModel execute(final CreateUserCommand command) {
    ...
    ensureEmailIsNotRegistered(new UserEmail(command.email()));
    final UserModel savedUser = saveUserPort.save(UserApplicationMapper.fromCreateCommandToModel(command));
  ...
  private void ensureEmailIsNotRegistered(final UserEmail email) {
